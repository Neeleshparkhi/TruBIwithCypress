...
describe('TC_06_HomePageReportSortBy suit' , function(){
    ...
    it('TC_05_HomePageReportSortBy test', function(){
        ...
        loginPage.getUserLogin(this.data.adminLogin,this.data.adminPassword)
         ...
         //Get All Report
         homePage.getreportSortBy().each(($el, index, $list) =>{

            const text = $el.text()
            //cy.log(text)
            homePage.getreportSortByIds().eq(0).select(text)
            cy.wait(2000)
